...
new (function () {
	...
	function ROSBot(name, ip) {
		...
		this.getImageListService = new ROSLIB.Service({
			ros: this.ros,
			name: '/airos/display/get_img_list',
			serviceType: 'display/get_img_list'
		})
	...
	ext.connectBot = function (bot, ip) {
		...
		if (!found) {
			...
			setTimeout(function () {
				...
				getImages(botAux);
				setTimeout(() => {
					setBlocks(bot, audioList, imageList);
	...
	var setEmotionAux = function (bot, emotion, block, callback) {
		var robot = findBot(bot);
		var state;
	...
	ext.setEmotion = function (bot, emotion, block, callback) {
		if (block == 'block') {
			setEmotionAux(bot, emotion, true, callback);
		}
		else {
			setEmotionAux(bot, emotion, false, null);
			callback();
		}
	}

	function botSay(bot, text, moving, block, callback) {
		var robot = findBot(bot);
	...
	ext.sayTTS = function (bot, text, moving, block, callback) {
		if (block == 'block') {
			if (moving == 'moving')
				botSay(bot, text.toString(), true, true, callback);
			else
				botSay(bot, text.toString(), false, true, callback);
		}
		else {
			if (moving == 'moving')
				botSay(bot, text.toString(), true, false, null);
			else
				botSay(bot, text.toString(), false, false, null);
			callback();
		}
	}
	...
	function moveServo(bot, position, time, servo, callback) {
		...
		if (robot != null && !isNaN(position) && position != null) {
			...
			"{position: 1.0, servo: 'head_h', time: 0.0, type: '', wait: true}"
			var request = new ROSLIB.ServiceRequest({
	...
	var setColorAux = function (bot, r, g, b, time, block, callback) {
		var robot = findBot(bot);
	...
	ext.hearth = function (bot, r, g, b, time, block, callback) {
		if (block == 'block') {
			setColorAux(bot, r, g, b, time, true, callback);
		}
		else {
			setColorAux(bot, r, g, b, time, false, null);
			callback();
		}
	}
	...
	function play_image_ros(bot, image, block, callback) {
		var robot = findBot(bot);
		if (robot != null) {
			var imageRequest = new ROSLIB.ServiceRequest({
				file: image
			});

			robot.mouthDrawImage.callService(imageRequest, function (result1) {
				if (callback != null)
					callback();
			});
		}
	...
	ext.playImage = function (bot, sound, block, callback) {
		if (block == 'block') {
			play_image_ros(bot, sound, true, callback);
		}
		else {
			play_image_ros(bot, sound, false, null);
			callback();
		}
	}

	function play_sound_ros(bot, sound, block, callback) {
		var robot = findBot(bot);
		if (robot != null) {
	...
	ext.playSound = function (bot, sound, block, callback) {
		if (block == 'block') {
			play_sound_ros(bot, sound, true, callback);
		}
		else {
			play_sound_ros(bot, sound, false, null);
			callback();
		}
	}
	...
	function getImages(bot) {
		...
		if (robot != null) {
			...
			robot.getImageListService.callService(req, function (result1) {
				ls = result1.list;
				ls = ls.sort();
				imageList = ls;
			});
	...
	ext.mouthDrawImage = function (bot, costume, sprite, callback) {
		...
		if (window.XMLHttpRequest && robot != null) {// code for IE7+, Firefox, Chrome, Opera, Safari
	...
	var descriptor = {
		blocks: [
			...
			['w', '[A] %s state is %m.states %m.blocking', 'setEmotion', 'bot1', 'Normal', 'block'],
			...
			['w', '[A] %s moves head horizontal to %n %m.velocity %m.blocking', 'moveHeadH', 'bot1', 0.5, 'medium', 'block'],
			['w', '[A] %s moves head vertical to %n %m.velocity %m.blocking', 'moveHeadV', 'bot1', 0.5, 'medium', 'block'],
			['w', '[A] %s moves eyebrows to %n %m.velocity %m.blocking', 'moveEyebrows', 'bot1', 0.5, 'medium', 'block'],
			['w', '[A] %s moves eyes to %n %m.velocity %m.blocking', 'moveEyes', 'bot1', 0.5, 'medium', 'block'],
			['w', '[A] %s heart light to: red %n green %n blue %n in %n secs %m.blocking', 'hearth', 'bot1', 255, 255, 255, 1, 'block'],
		...
		menus: {
			...
			states: ['Normal', 'Sad', 'Happy', 'Angry', 'Indifferent', 'Surprise', 'Disgust', 'Relief', 'Reproach', 'Pride', 'Admiration', 'Scared', 'Sleep', 'Meltdown', 'Overstimulated', 'Repulsed', 'Excited', 'Calm', 'Confused', 'NoEmotion'],
			saying: ['with', 'without'],
	...
	function setBlocks(name, audio_list, image_list) {
		descriptor2 = {
			blocks: [
				['w', '[A] %s draws %m.imageList %m.blocking', 'playImage', name, '', 'block'],
				['w', '[A] %s plays %m.soundList sound with %m.blocking', 'playSound', name, '', 'block'],
